...
dplyr,scales,patchwork,plotly,igraph,ggraph,ggplot2)
...
ui <- fluidPage(
  titlePanel("Influence Network Explorer"),
  sidebarLayout(
    sidebarPanel(
      selectInput(
        inputId  = "target",
...
server <- function(input, output, session) {
  ...
  output$net_plot <- renderPlot({
    ...
    ego_vs <- unique(c(
      root_idx,
      igraph::neighbors(g_ig, root_idx, mode = "in"),
      igraph::neighbors(g_ig, root_idx, mode = "out")
    ))

    
    subg <- igraph::induced_subgraph(g_ig, vids = ego_vs)

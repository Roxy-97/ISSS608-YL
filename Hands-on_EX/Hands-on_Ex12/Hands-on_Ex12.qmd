...
![](images/clipboard-3684533947.png)
...
::: {.callout-note style="backgroud-color:white;" icon="false"}
...
![You can display the basic information of the newly created sgpools_sf by using the code chunk below.](images/clipboard-83711665.png)
...
The code chunks below show that the variable ***Gp1Gp2Winnings*** is assigned to size visual attribute.
...
::: {.callout-important style="background-color:white;" icon="false"}
## tm_bubbles() vs tm_facets():

**tm_bubbles()**\
- Plots “bubble” symbols at each point location\
- Maps bubble size to a numeric variable (e.g. population, earnings)\
- Allows you to style fill color, border color, and line width

**tm_facets()**\
- Splits the map into multiple panels (“small multiples”) by a categorical field\
- Controls layout with `nrow`/`ncol`\
- `sync = TRUE` keeps all panels’ zoom/pan in sync for easy comparison
:::

Notice: B**efore you end the session**, it is wiser to switch **tmap**’s Viewer back to **plot mode** by using the code chunk below.
